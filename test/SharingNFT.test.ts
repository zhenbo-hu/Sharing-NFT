...
describe("SharingNFT", function () {
  ...
  describe("mint", function () {
    ...
    it("mint with empty tag", async function () {
      await expect(contract.mint("http://localhost", "")).to.be.revertedWith(
        "tag cannot be empty."
      );
    });
  ...
  describe("burn", function () {
    it("burn by its owner", async function () {
      ...
      await expect(contract.connect(otherUser).burn(1))
    ...
    it("burn by others and revert", async function () {
      ...
      await expect(contract.connect(owner).burn(1)).to.be.revertedWith(
